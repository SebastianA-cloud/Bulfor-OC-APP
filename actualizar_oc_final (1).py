...
def cargar_hospitales_maestro():
    """Carga el diccionario RUT -> nombre_estandar desde Supabase.
    Así cuando llega una OC nueva con RutUnidad, usamos el nombre
    estandarizado que el usuario ya definió, no el que viene de la API."""
    try:
        res = supabase.table("hospitales_maestro").select("rut,nombre_estandar").execute()
        return {r["rut"]: r["nombre_estandar"] for r in (res.data or []) if r.get("rut") and r.get("nombre_estandar")}
    except Exception as e:
        print(f"⚠ No se pudo cargar hospitales_maestro: {e}")
        return {}


def transformar_oc(detalle, codigo_oc, hospitales_por_rut=None):
    """Equivalente exacto a Mis_OC_API: ExpandirOC + ExpandirFechas +
    ExpandirComprador + ExpandirProveedor + AgregarTipoOC.
    Si se pasa hospitales_por_rut, normaliza el nombre usando el RUT."""
    ...
    # Intentamos normalizar el nombre del hospital usando el RUT
    rut_unidad = comprador.get("RutUnidad")
    nombre_mp = comprador.get("NombreOrganismo")
    if hospitales_por_rut and rut_unidad and rut_unidad in hospitales_por_rut:
        nombre_hospital = hospitales_por_rut[rut_unidad]
        if nombre_hospital != nombre_mp:
            print(f"    ✎ Normalizando hospital: '{nombre_mp}' → '{nombre_hospital}'")
    else:
        nombre_hospital = nombre_mp  # respaldo: usamos el nombre de la API
...
def main():
    ...
    # Cargamos el diccionario RUT -> nombre_estandar para normalizar hospitales
    print("Cargando diccionario de hospitales maestro...")
    hospitales_por_rut = cargar_hospitales_maestro()
    print(f"  {len(hospitales_por_rut)} hospitales con nombre estandarizado\n")
    ...
    while fecha_actual.date() <= hoy.date():
        ...
        for oc_basica in codigos_basicos:
            ...
            oc_transf = transformar_oc(detalle, codigo, hospitales_por_rut)
