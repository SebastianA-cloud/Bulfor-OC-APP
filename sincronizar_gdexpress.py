...
def parse_fecha(texto):
    if not texto:
        return None
    # GDExpress no es consistente: algunos campos traen fecha+hora
    # ("2024-02-01T00:00:00") y otros (como FchVenc) traen SOLO la fecha
    # ("2024-02-01") — sin esto, cualquier campo del segundo tipo fallaba
    # en silencio y el código caía al siguiente campo disponible.
...
def documentos_desde_xml(xml_bytes, imprimir_diagnostico=False):
    ...
    for i, doc in enumerate(root.findall('document')):
        ...
        # FchVenc es el campo OFICIAL del SII para el vencimiento (el mismo
        # que se ve impreso en el documento). DueDate es un cálculo propio
        # de GDExpress que a veces no coincide con el plazo real acordado
        # con el proveedor — por eso se prioriza FchVenc, y solo se usa
        # DueDate como respaldo si por algún motivo no viene FchVenc.
